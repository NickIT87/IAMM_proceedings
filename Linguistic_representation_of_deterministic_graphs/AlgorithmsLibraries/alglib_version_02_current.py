...
def get_leaf_nodes_from_dgraph(dgraph: nx.Graph) -> dict:
...
def get_node_id_by_word(graph: nx.Graph, word: str, root_node: int) -> int:
    ...
    for symbol in word[1:]:
        ...
        labels = [graph.nodes[node_id]['label'] for node_id in neighbors]
        try:
...
def ar_nodes(graph: nx.Graph) -> nx.Graph:
    ...
    while trigger:
        ...
        for node in G_.nodes:
            ...
            labels = [G_.nodes[node_id]['label'] for node_id in neighbors]
            equals_labels = find_neighbours_with_the_same_labels(
                neighbors, labels
            )
            if equals_labels:
